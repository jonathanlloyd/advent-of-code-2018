...
if __name__ == '__main__':
    ...
    total_sleep_by_guard = {
        guard_id: sum(minute_counts.values())
        for guard_id, minute_counts in minute_count_by_guard.items()
    }
    sleepiest_guard_id, _ = max(total_sleep_by_guard.items(), key=lambda x: x[1])

    minute_counts = minute_count_by_guard[sleepiest_guard_id]
    sleepiest_minute, _ = max(minute_counts.items(), key=lambda x: x[1])

    answer = sleepiest_guard_id * sleepiest_minute
